use crate::models::backup::{CloudBackupFile, CommandResult, ProgressPayload};
use regex::Regex;
use reqwest::Client;
...
pub async fn list_files(
    ...
    let re_size =
        Regex::new(r"<[\w:]*getcontentlength[^>]*>(\d+)</[\w:]*getcontentlength>").unwrap();
    ...
    for cap in re_response.captures_iter(&body) {
        ...
        let decoded_path = urlencoding::decode(&full_path)
            .unwrap_or(std::borrow::Cow::Borrowed(&full_path))
            .to_string();
